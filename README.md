...
- **LLM Guard Project** (`llm_demo_project/`): A self-contained tutorial for evaluating AI applications (chatbots, summarizers) using LLM-as-a-Judge. It includes a **Mock Executor** that simulates GPT-4 behavior, allowing you to run the full evaluation workflow (properties, cost monitoring, adoption gating) without needing API keys. Ideal for first-time users wanting to explore LLM features risk-free.
